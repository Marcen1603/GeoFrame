...
from HelperFunctions import extract_osm_statistics, get_min_max_lon_lat, print_to_console, delete_file, calc_file_size_gb
...
class Preprocessor:
    ...
    def split_file(self, args):
        ...
        # Create sub-file
        path_to_new_file = self.create_sub_file(raw_file_path, new_lon_min, new_lat_min, new_lon_max, new_lat_max)
    ...
    def sub_files(self, file_size_gb:float, statistics_dict:dict, raw_file_path:str):
        ...
        lon_min, lon_max, lat_min, lat_max = get_min_max_lon_lat(statistics_dict)
        ...
        for x in range(split_size):
            ...
            for y in range(split_size):
                args_list.append((x, y, split_size, lat_min, latitude_split, lon_min, longitudinal_split, raw_file_path))
        ...
        while len(process_files) > 0:
            for process_file in process_files:
                ...
                if file_size_gb > self.max_split_size:
                    ...
                    self.sub_files(file_size_gb, statistics_dict, path_to_process_file)
