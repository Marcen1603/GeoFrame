...
class Generator:
    ...
    def __init__(self, lat: float, lon: float, dis: int):
        ...
        self.path_to_latest_continents = os.path.join(self.path_to_latest, 'continents')
        self.path_to_latest_preprocessed = os.path.join(self.path_to_latest, 'preprocessed')
        ...
        osmconvert_path = self.path_to_osm_convert_linux if OS.from_str(platform.system()) == OS.LINUX else self.path_to_osm_convert
    ...
    def select_pbf_file(self, bbox_dict: dict) -> str:
        ...
        path_to_preprocessed_cache_file = os.path.join(self.path_to_latest_preprocessed, 'cache_file*.json')
        path_to_continent_cache_file = os.path.join(self.path_to_latest_continents, 'cache_file*.json')
        
        found = None
        for cache_file in [path_to_preprocessed_cache_file, path_to_continent_cache_file]:
            ...
            if len(cache_files) == 1:
                ...
                raise ValueError(f'Found no or multiple cache files in: {path_to_preprocessed_cache_file}')
